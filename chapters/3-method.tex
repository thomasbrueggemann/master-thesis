...
Generic strategies are strategies contain algorithms that are configurable and usable by other specific strategies.
...
A specific strategy on the other hand directly targets the exploration of an \ipp and contains the \ipp hierarchy identifier, introduced by \textcite{Dehling2016}, in its Java-classname.\footnote{See \cite{Dehling2016}, p. 6.}
The Java-class \textit{analyze.src.strategies.CI213Strategy} contains a search pattern for the \ipp with the hierarchy identifier CI213, which refers to the \ipp Content (C) \rightarrow{} InformationCollectionContent (I) \rightarrow InformationCollectionSensorContent (2) \rightarrow EnvironmentSensorContent (1) \rightarrow MicrophoneContent (3).
